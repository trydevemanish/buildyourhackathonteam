...
export default function TeamsCreatedByUser() {
    ...
    const { data : teamDataFromBackend,errors,loading } = useFetchData<TeamCardInfoType[]>({ url:'/api/fetchallteamCreatedbyuser' })
    const { errors: userExitsError } = useFetchData<any>({ url:'/api/findandcreateUser'})

    if(userExitsError){
    console.log(`Error checking user exits: ${errors}`)
    }
    ...
    if(teamDataFromBackend){
        console.log('teamDataFromBackend',teamDataFromBackend)
    }
