            ...
            layoutDataNavigationStructure.map((data : layoutDataNavigationStructure,idx : number) => (
              <p 
                className={`cursor-pointer font-semibold 
                  ${selected.current && selected.current == data?.id ? 'bg-purple-300' : ''}
                hover:bg-purple-300 py-1 px-2 rounded`} 
                onClick={async() => await handleMenuSelect(data?.pushto,data?.id)} key={idx} >
                  {data?.para}
              </p>
            ))
          }
