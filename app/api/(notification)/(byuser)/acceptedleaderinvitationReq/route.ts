...
export async function PUT(req:Request) {
    try {
        ...
        const updateStatus = await prisma.leaderReqUserToJoinThereTeam.update({
            where : {
                id : reqid,
                teamid : teamid,
                leaderid : leaderid,
        ...
        return NextResponse.json(
            {message:'User accepted invitation, user added to team'},
            {status:200}
        )

        
    } catch (error) {
        console.log(`Failed to accept leader req: ${error}`)
        return NextResponse.json(
            {error:`Failed to accept leader req: ${error}`},
            {status:500}
        )
    }
}